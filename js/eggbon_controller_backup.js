...
$(function () {
    EggbonEditor.controller = {
        ...
        handleTreeMoveEvent: function (event) {
            event.preventDefault();
            console.log('moved_node : ', event.move_info.moved_node.data.id);
            console.log('target_node : ', event.move_info.target_node.data.id);
		...
		movePageAfter : function(treeEvent, target_comp, moved_comp){
			var moved_comp_index = -1;
			$.each(EggbonEditor.project.pages, function(inx, page) {
				if (moved_comp.UUID == page.UUID) {
					moved_comp_index = inx;
					return;
				}
			});

			var target_comp_index = -1;
			$.each(EggbonEditor.project.pages, function(inx, page) {
				if (target_comp.UUID == page.UUID) {
					target_comp_index = inx;
					return;
				}
			});
			
			console.log(moved_comp_index + " : " + target_comp_index);
			if (target_comp_index != -1 && target_comp_index != -1) {
				EggbonEditor.project.pages.splice(moved_comp_index, 1);
				EggbonEditor.project.pages.splice(target_comp_index, 0, moved_comp);
				
				$(target_comp.externalWrapperQueryStr).before($(moved_comp.externalWrapperQueryStr));
				treeEvent.move_info.do_move();
				
				var node = this.$tree.tree("getNodeById", moved_comp.UUID);
				this.$tree.tree('selectNode', node);
				EggbonEditor.showPage(moved_comp.UUID, moved_comp.name);
			}
		},  
